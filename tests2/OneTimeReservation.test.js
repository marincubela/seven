...
chrome.setDefaultService(
  new chrome.ServiceBuilder(
    "C:/Program Files (x86)/ChromeDriver/bin/chromedriver.exe"
  ).build()
);
...
describe("Registration company test", () => {
  it("Wrong registration", async () => {
    ...
    await driver.wait(
      until.elementLocated(By.css('input[name="login-email"]'))
    );
    ...
    await driver
      .actions()
      .click(emailInput)
      .sendKeys("klijent", String.fromCharCode(64), "mail.com")
      .perform();

    await driver.wait(
      until.elementLocated(By.css('input[name="login-password"]'))
    );

    await (
      await driver.findElement(By.css('input[name="login-password"]'))
    ).sendKeys("12345678");
    ...
    await driver.wait(
      until.elementLocated(By.css('input[name="add-vehicle-name"]'))
    );
    await (
      await driver.findElement(By.css('input[name="add-vehicle-name"]'))
    ).sendKeys("Auto");

    await driver.wait(
      until.elementLocated(By.css('input[name="add-vehicle-registration"]'))
    );
    await (
      await driver.findElement(By.css('input[name="add-vehicle-registration"]'))
    ).sendKeys("1234 ZG");
    ...
    const actual = (await driver.getCurrentUrl()).includes(
      "app.parkirajme.xyz/vehicles"
    );
